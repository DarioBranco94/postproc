...
class Checker:
    ...
    def checkChargingPowerLowerThanMaxChPowConstraint(self, startTime):
        ...
        for key, ev in self.evList.items():
            filename = ev.profile
            powerProfile = generateEnergyTimeSeries(filename, startTime)
            respected = 1
            ...
            for ecar in elem:  # car
                if (ecar.tag == "ecar"):
                    ...
                    try:
                        ...
                        self.evList[tempo].departureTimeMinusArrivalTimeMinusEnergyDemand = -float(
                            ecar.find("capacity").text) / float(ecar.find("maxchpowac").text)
                    except:
                        None
    ...
    with open(file, newline='') as csvfile:
        ...
        for row in csv_reader:  # per tutte le righe
            ...
            if (count != 0):  # salto la prima riga del
                ...
                y.append(3.6*(float(row[1]) - lastValue) / (float(row[0]) - lastSample))
                x.append(float(row[0]))  # aggiunto il tempo alla lista dei tempi
                y.append(3.6 * (float(row[1]) - lastValue) / (float(row[0]) - lastSample))
...
if __name__ == "__main__":
    ...
    checker.doChecks("./08_16_21_11/output", 1629072000,"./08_16_21_11/xml","./10_08_16_1")
